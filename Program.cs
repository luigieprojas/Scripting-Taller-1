using System;
using System.Collections.Generic;

namespace TallerScripting
{
   
    public class Estudiante
    {
        public string Nombre { get; set; }
    }

    class Program
    {
        static void Main(string[] args)
        {

            // --- BLOQUE DE INVOCACIONES ---

            // 1. Matriz (Ref: Punto 1 PDF)
            CorregirMatriz(new int[,] { { -1, 2 }, { -3, 4 } });

            // 2. Arreglo Flotantes (Ref: Punto 2 PDF)
            float p = PromedioPositivos(new float[] { 1.2f, 3.4f });

            // 3. Matriz Cadenas (Ref: Punto 3 PDF)
            string s = UnirMatriz(new string[,] { { "Hola", "Profe" } });

            // 4. Ciclos (Ref: Punto 6 PDF)
            int sumaI = SumarImpares(10);

            // 5. Firma Misteriosa (Ref: Punto 13 PDF)
            FuncionMisteriosa('A', 'B', "CD", 'E', 'F', "GHI");

            // 6. Firma WTF (Ref: Punto 13 PDF)
            int[] w = WTF("Scripting", true, 99.9);

            // 7. Booleano (Ref: Punto 15 PDF)
            bool esPal = EsPalindromo("reconocer");

            // 8. Concatenación (Ref: Punto 16 PDF)
            string textounido = Concatenar("C# ", "es genial");

            // 9. Conversión de tipos
            double num = Fraccionar("15.75");

            // 10. Invertir Arreglo
            InvertirArray(new int[] { 5, 4, 3 });

            // 11. Búsqueda en Matriz
            BuscarEnMatriz(new int[2, 2], 7);

            // 12. Sin parámetros ni retorno
            Saludar();

            // 13. Uso de Objeto (Clase Estudiante)
            ProcesarAlumno(new Estudiante { Nombre = "Estudiante" });

            // 14. Lista Dinámica (Colecciones)
            List<int> lista = GenerarLista();

            // 15. Validación de Texto
            bool val = ValidarCadena("456");

            // 16. Factorial (Ref: Punto 29 PDF)
            long fact = CalcularFactorial(5);

            // 17. Retorno de carácter (Char)
            char letra = ObtenerInicial("Visual Studio");

            // 18. Múltiples parámetros (Fecha)
            string hoy = FormatearFecha(29, 01, 2024);

            // 19. Función vacía técnica
            ProcesoInterno();

            // 20. Suma múltiple (Diferentes tipos de datos)
            int total = SumarCuatro(10, 20, 30, 40);

            Console.ReadKey(); // Para que no se cierre la consola
        }
        // --- BLOQUE DE FIRMAS (DEFINICIONES) CON COMPROBANTES ---

        // 1. Matriz
        static void CorregirMatriz(int[,] m) { Console.WriteLine("1. Matriz funciona."); }

        // 2. Arreglo Flotantes
        static float PromedioPositivos(float[] a) { Console.WriteLine("2. Promedio funciona."); return 0.0f; }

        // 3. Cadena Matriz
        static string UnirMatriz(string[,] m) { Console.WriteLine("3. Cadena funciona."); return "Unido"; }

        // 4. Ciclos
        static int SumarImpares(int n) { Console.WriteLine("4. Ciclo funciona."); return 0; }

        // 5. Mixto
        static void FuncionMisteriosa(char a, char b, string c, char d, char e, string f) { Console.WriteLine("5. Misteriosa funciona."); }

        // 6. Complejo
        static int[] WTF(string s, bool b, double d) { Console.WriteLine("6. WTF funciona."); return new int[0]; }

        // 7. Booleano
        static bool EsPalindromo(string s) { Console.WriteLine("7. Palíndromo funciona."); return true; }

        // 8. String
        static string Concatenar(string a, string b) { Console.WriteLine("8. Concatenar funciona."); return a + b; }

        // 9. Conversión
        static double Fraccionar(string s) { Console.WriteLine("9. Doble funciona."); return 0.0; }

        // 10. Invertir
        static void InvertirArray(int[] a) { Console.WriteLine("10. Invertir funciona."); }

        // 11. Búsqueda
        static void BuscarEnMatriz(int[,] m, int n) { Console.WriteLine("11. Buscar funciona."); }

        // 12. Simple
        static void Saludar() { Console.WriteLine("12. Saludo funciona."); }

        // 13. Objetos
        static void ProcesarAlumno(Estudiante e) { Console.WriteLine("13. Objeto funciona."); }

        // 14. Listas
        static List<int> GenerarLista() { Console.WriteLine("14. Lista funciona."); return new List<int>(); }

        // 15. Validación
        static bool ValidarCadena(string s) { Console.WriteLine("15. Validar funciona."); return true; }

        // 16. Matemática
        static long CalcularFactorial(int n) { Console.WriteLine("16. Factorial funciona."); return 1; }

        // 17. Char
        static char ObtenerInicial(string s) { Console.WriteLine("17. Inicial funciona."); return s[0]; }

        // 18. Fecha
        static string FormatearFecha(int d, int m, int a) { Console.WriteLine("18. Fecha funciona."); return $"{d}/{m}/{a}"; }

        // 19. Vacía
        static void ProcesoInterno() { Console.WriteLine("19. Vacío funciona."); }

        // 20. Suma
        static int SumarCuatro(int a, int b, int c, int d) { Console.WriteLine("20. Suma funciona."); return a + b + c + d; }
    }
}

/* TEORÍA:
   
   1. ¿Qué es un ciclo infinito?: 
      Es un bucle cuya condición de finalización nunca se cumple, causando que el 
      programa se ejecute indefinidamente. Ejemplo: while(true) { }.

   2. ¿Qué es un error en tiempo de ejecución?: 
      Es un error que no es detectado por el compilador (la sintaxis es correcta) 
      pero ocurre mientras el programa corre. Ejemplo: Dividir por cero.
*/
